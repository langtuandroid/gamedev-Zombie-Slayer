            ...
            foreach (var hit in hits)
            {
                if (counterHit < maxTargetPerHit)
                {
                    ...
                    if (takeDamage != null)
                    {
                        ...
                        {
                            takeDamage.TakeDamageE(_damage, Vector2.zero, hit.point, gameObject, BODYPART.NONE, hasWeaponEffect);
                        }
                        else
                            takeDamage.TakeDamageE(_damage, Vector2.zero, hit.point, gameObject);
