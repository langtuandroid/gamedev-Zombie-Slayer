...
public class BezierCollider2DEditor : Editor
{
    ...
    void OnSceneGUI()
    {
        if (bezierCollider != null)
        {
            ...
            var fmh_69_154_638479559903564892 = Quaternion.identity; bezierCollider.handlerSecondPoint = Handles.FreeMoveHandle(bezierCollider.transform.position + ((Vector3)bezierCollider.handlerSecondPoint), 0.04f * HandleUtility.GetHandleSize(bezierCollider.transform.position + ((Vector3)bezierCollider.handlerSecondPoint)), Vector3.zero, Handles.DotHandleCap) - bezierCollider.transform.position;
